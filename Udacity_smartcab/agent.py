...
class LearningAgent(Agent):
    ...
    def build_state(self):
        ...
        #str_func = lambda x: 'None' if x is None else str(x)
        
        #state = [str_func(st) for st in state]
        ...
        if not self.learning:
            ...
            for k in self.Q[state].keys():
                if self.Q[state][k] == self.get_maxQ(state):
                    maxQ_tie_breaker.append(k)
            action = random.choice(maxQ_tie_breaker) 
